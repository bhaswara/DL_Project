...
class Generator(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.inc1 = nn.InstanceNorm2d(128)
        self.inc2 = nn.InstanceNorm2d(256)
        self.inc3 = nn.InstanceNorm2d(512)
        self.inc4 = nn.InstanceNorm2d(512)
        self.inc5 = nn.InstanceNorm2d(512)
        self.inc6 = nn.InstanceNorm2d(512)

        self.ind7 = nn.InstanceNorm2d(512)
        self.ind6 = nn.InstanceNorm2d(512)
        self.ind5 = nn.InstanceNorm2d(512)
    ...
    def forward(self, x):           
        en0 = self.c0(x)
        en1 = self.inc1(self.c1(F.leaky_relu(en0, negative_slope=0.2)))
        en2 = self.inc2(self.c2(F.leaky_relu(en1, negative_slope=0.2)))
        en3 = self.inc3(self.c3(F.leaky_relu(en2, negative_slope=0.2)))
        en4 = self.inc4(self.c4(F.leaky_relu(en3, negative_slope=0.2)))
        en5 = self.inc5(self.c5(F.leaky_relu(en4, negative_slope=0.2)))
        en6 = self.inc6(self.c6(F.leaky_relu(en5, negative_slope=0.2)))
        en7 = self.c7(F.leaky_relu(en6, negative_slope=0.2))

        de7 = self.ind7(self.d7(F.relu(en7)))
        de6 = F.dropout(self.ind6(self.d6(F.relu(torch.cat((en6, de7),1)))))
        de5 = F.dropout(self.ind5(self.d5(F.relu(torch.cat((en5, de6),1)))))

        de4 = F.dropout(self.ind4(self.d4(F.relu(torch.cat((en4, de5),1)))))
        de3 = self.ind3(self.d3(F.relu(torch.cat((en3, de4),1))))
        de2 = self.ind2(self.d2(F.relu(torch.cat((en2, de3),1))))
        de1 = self.ind1(self.d1(F.relu(torch.cat((en1, de2),1))))
